            ...
            <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className="list-none mt-4">
                {footerLink.links.map((link, i) => (
                  <li key={link.name} className={`font-poppins font-noraml text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${i !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'} ` }>
                    {link.name}
                    </li>
                ))}
              </ul>
    ...
    <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]">
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
        {socialMedia.map((social, i) => (
          <img key={social.id} src={social.icon} className={`w-[21px] h-[21px] object-contain cursor-pointer ${socialMedia.length - 1 !== i ? 'mr-6' : 'mr-0'}`} />
        ))}
      </div>
    </div>
  </section>
